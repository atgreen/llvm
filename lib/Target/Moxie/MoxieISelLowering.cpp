//===-- MoxieISelLowering.cpp - Moxie DAG Lowering Implementation  --------===//
//
...
MoxieTargetLowering::MoxieTargetLowering(const TargetMachine &TM,
                                     const MoxieSubtarget &STI)
    : TargetLowering(TM) {
#if 0
  // Set up the register classes.
  addRegisterClass(MVT::i64, &Moxie::GPRRegClass);

  // Compute derived properties from the register classes
  computeRegisterProperties(STI.getRegisterInfo());

  setStackPointerRegisterToSaveRestore(Moxie::R11);
...
SDValue MoxieTargetLowering::LowerOperation(SDValue Op, SelectionDAG &DAG) const {
#if 0
  switch (Op.getOpcode()) {
  case ISD::BR_CC:
...
SDValue MoxieTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
                                     SmallVectorImpl<SDValue> &InVals) const {
#if 0
  ...
  MachineFunction &MF = DAG.getMachineFunction();
  // Moxie target does not support tail call optimization.
  IsTailCall = false;
  ...
  CCInfo.AnalyzeCallOperands(Outs, CC_Moxie64);
  ...
  Chain = DAG.getNode(MoxieISD::CALL, CLI.DL, NodeTys, Ops);
  InFlag = Chain.getValue(1);
...
MoxieTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
  ...
  return DAG.getNode(MoxieISD::RET, DL, MVT::Other,
		     Chain, DAG.getRegister(Moxie::FP, MVT::i32));
#if 0
  FIXME
  SmallVector<SDValue, 4> RetOps(1, Chain);
  unsigned Opc = MoxieISD::RET;
...
SDValue MoxieTargetLowering::LowerBR_CC(SDValue Op, SelectionDAG &DAG) const {
#if 0
  ...
  return DAG.getNode(MoxieISD::BR_CC, DL, Op.getValueType(), Chain, LHS, RHS,
...
SDValue MoxieTargetLowering::LowerSELECT_CC(SDValue Op, SelectionDAG &DAG) const {
#if 0
  ...
  return DAG.getNode(MoxieISD::SELECT_CC, DL, VTs, Ops);
#endif
  assert(0 && "unimplemented");
  return Op;
}
...
SDValue MoxieTargetLowering::LowerGlobalAddress(SDValue Op,
  ...
  return DAG.getNode(MoxieISD::Wrapper, DL, MVT::i64, GA);
#endif
  assert(0 && "unimplemented");
  return Op;
}
...
MachineBasicBlock *
MoxieTargetLowering::EmitInstrWithCustomInserter(MachineInstr *MI,
                                               MachineBasicBlock *BB) const {
#if 0
  const TargetInstrInfo &TII = *BB->getParent()->getSubtarget().getInstrInfo();
  DebugLoc DL = MI->getDebugLoc();

  assert(MI->getOpcode() == Moxie::Select && "Unexpected instr type to insert");
  ...
  switch (CC) {
  case ISD::SETGT:
    BuildMI(BB, DL, TII.get(Moxie::JSGT_rr))
        .addReg(LHS)
        .addReg(RHS)
        .addMBB(Copy1MBB);
    break;
  case ISD::SETUGT:
    BuildMI(BB, DL, TII.get(Moxie::JUGT_rr))
        .addReg(LHS)
        .addReg(RHS)
        .addMBB(Copy1MBB);
    break;
  case ISD::SETGE:
    BuildMI(BB, DL, TII.get(Moxie::JSGE_rr))
        .addReg(LHS)
        .addReg(RHS)
        .addMBB(Copy1MBB);
    break;
  case ISD::SETUGE:
    BuildMI(BB, DL, TII.get(Moxie::JUGE_rr))
        .addReg(LHS)
        .addReg(RHS)
        .addMBB(Copy1MBB);
    break;
  case ISD::SETEQ:
    BuildMI(BB, DL, TII.get(Moxie::JEQ_rr))
        .addReg(LHS)
        .addReg(RHS)
        .addMBB(Copy1MBB);
    break;
  case ISD::SETNE:
    BuildMI(BB, DL, TII.get(Moxie::JNE_rr))
        .addReg(LHS)
        .addReg(RHS)
        .addMBB(Copy1MBB);
    break;
  ...
  // ...
  BB = Copy1MBB;
  BuildMI(*BB, BB->begin(), DL, TII.get(Moxie::PHI), MI->getOperand(0).getReg())
